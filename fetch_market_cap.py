...
# Pre-filter query sent to Screener — keeps CSV small by excluding micro-caps.
# This is NOT the strategy's 1,500–5,000 Cr band; that filter stays in
# prepare_data.py unchanged.  Screener syntax: plain English expressions.
...
OUT_PATH    = MCAP_DIR / f"market_cap_today_{TODAY}.csv"
...
def _login(session: requests.Session) -> None:
    ...
    if "/login/" in resp.url or "Please enter a correct" in resp.text:
        raise ValueError(
            "Login FAILED — check SCREENER_EMAIL / SCREENER_PASSWORD in fetch_market_cap.py"
        )
...
def _find_latest_fallback():
    ...
    files = sorted(MCAP_DIR.glob("market_cap_today_*.csv"))
...
def main() -> int:
    ...
    try:
        ...
        if fallback:
            fallback_date = fallback.stem.replace("market_cap_today_", "")
            msg = (
            ...
            print(f"  WARNING: market cap data is from {fallback_date} — "
                  f"treat today's signals with extra caution.")
        ...
        msg = (
            f"Screener.in fetch failed ({type(exc).__name__}: {exc}) "
            f"and no previous market_cap_today_*.csv found. "
            f"prepare_data.py will fall back to semi-annual NSE snapshots."
        )
        print(f"  NO FALLBACK AVAILABLE.")
        print(f"  prepare_data.py will use semi-annual NSE snapshots instead.")
